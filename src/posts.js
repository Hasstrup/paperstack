...
this.socket = SocketIOClient('https://paperstack.ml')
    ...
    this.socket.on('update', () => {
      axios.get('https://paperstack.ml/api/posts')
      .then((response) => {
...
static navigationOptions =  ({navigation}) => {
  ...
  marginLeft: 2,
    ...
    axios.get('https://paperstack.ml/api/posts')
    .then((response) => {
      this.setState({refreshing: false, posts: response.data.posts});
    });
  }
      ...
      if(this.state.linking === 'false') {
        modalcontent = (
          ...
          <TouchableWithoutFeedback style={{ width: 30, height: 20, position: 'relative', top: 32, left: -6}} onPress={() => this.rateme()}>
            <Icon name='heart' type='evilicon' color={this.state.pressed === 'false' ? '#48C9B0' : '#EC7063'} size={this.state.pressed === 'false' ? 25 : 30} />
    ...
    return(
      <View style={styles.container}>
        ...
        <View style={{ flex: 1, position: 'absolute', bottom: 0, opacity: 0}}>
          <AdMobBanner
            adSize="fullBanner"
            adUnitID="ca-app-pub-6762059104295133/1787899674"
            onAdFailedToLoad={error => console.error('hi i am posts'+ error)}
            />
        </View>
      </View>
